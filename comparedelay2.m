...
%  the DELAYESTTM and DELAYEASTFR functions) .  Signal simulation values include
...
%  errors for both approaches presented in the titles of the figures.
...
dl = .001;  %  Simulated Delay in seconds
...
prot.interpord = 4;  %  Half the Sinc function interoplation order
prot.interpfac = 10; %  Number of interpolated points between samples (factor to decrease grid spacing)

% Signal Processing parameters for frequecy domain approach
...
f12s = f12p*(1+xsp/100);  %  Upper stopband fop simulated signal
...
for k=1:runs
    ...
    %  Randomly place pulse in processin window
         [mx, rmax] = max(abs(s2));  %  Find sample with strongest peak center window there
         edpoint = rmax(1) - 1 + round(winlen/2);  % Initial signal window index 
         stpoint = edpoint - winlen;
         smat1 = [smat1, s1(stind:edind)];
         smat2 = [smat2, s2(stind:edind)];
end
    %  Apply delay estimators
%    [df, stds, prof] = delayestfr(smat2,smat1,fs,prof);
    [dt, cv, prot] = delayesttm(smat2,smat1,fs, prot);
    %  Comment out next 10 lines to avoid the plotting and stoping in the
...
%     title(['delay estimate (frequency domain) = ' num2str(1000*df(k)) 'ms  and % error = ' num2str(100*(df(k) - dl)/dl) ])
%     xlabel('milliseconds')
%     figure(2); plot(1000*[stind:edind]/fs, s2(stind:edind),'r');
%     hold on; plot(1000*[stind:edind]/fs, s1(stind:edind),'g'); hold off
%     title(['delay estimate (time domain) = ' num2str(1000*dt(k)) 'ms  and % error = ' num2str(100*(dt(k) - dl)/dl) ])
%     xlabel('milliseconds')
