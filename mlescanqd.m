function [mleim] = mlescanqd(sigar, envpar, procpar)
...
%             pause
           dumval = zeros(1,sc*(sc-1)*(sc-2)+sc*(sc-1));
           icn = 1; 
           for kmps=1:sc-3
                parray = conj(temp(:,kmps));
                 for da =kmps+1:sc-2
                     tarray = parray.*temp(:,da);
                     dumval(icn) = sum(tarray);
                     icn=icn+1;
                     for ta=da+1:sc-1
                         parray = conj(temp(:,ta));
                         for fa=ta+1:sc
                             farray = parray.*temp(:,fa);
                             dumval(icn) = sum(farray);
                             icn=icn+1;
                             dumval(icn) = (sum(farray.*tarray));
                             icn=icn+1;
                         end
                     end
                 end
